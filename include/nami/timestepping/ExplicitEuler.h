...
namespace nami::timestepping
{
  ...
  template <typename T>
  concept State_t = true;

  template <typename T, typename State>
  concept Rhs_t = std::invocable<T, Time_t, State>;

  template <typename T, typename State>
  concept RhsProxy_t = std::invocable<T, Time_t, State, IntegrationStep_t>;

  template <typename T, typename State>
  concept TimeIntegrator_t = std::invocable<T, Time_t, Timestep_t, State, IntegrationStep_t>;

  struct ExplicitEuler {
    template <State_t State>
    inline constexpr void operator()(Time_t time, Timestep_t timestep, State&& state,
